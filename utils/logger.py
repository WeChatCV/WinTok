...
def tensorboard_log(tb_logger, data, step=None, log_freq=None):
    """Log data to TensorBoard"""
    if not dist.is_master() or tb_logger is None:
        return
    if step is not None and log_freq is not None:
        if step % log_freq == 0:
            tb_logger.update(step=step, **data)
        else:
            return
    else:
        tb_logger.update(step=step, **data)
...
def setup_tensorboard_logger(args: config.Args):
    """Setup TensorBoard logger"""
    if not args.use_tensorboard:
        return None
    
    if dist.is_master():
        log_dir = os.path.join(args.tensorboard_log_dir, args.exp_name)
        os.makedirs(log_dir, exist_ok=True)
        tb_logger = misc.TensorboardLogger(
            log_dir=log_dir,
            filename_suffix=f'_{args.run_id or "default"}'
        )
        return misc.DistLogger(tb_logger)
    else:
        return misc.DistLogger(None)
