...
class Args(Tap):
    model: str = '/mnt/shenzhen2cephfs/mm-base-vision/markywguo/models/siglip2-so400m-patch16-256'  # siglip2-so400m-patch16-256 / siglip2-so400m-patch14-384
    exp_name: str = 'wintok'
    ...
    fid_eval_src: str = '/mnt/shenzhen2cephfs/mm-base-vision/markywguo/dataset/ImageNet-1k/imagenet_256_val/'
    fid_eval_dst: str = 'eval_recon'
    vis_img_dir: str = 'assets/vis_imgs/'
    ...
    grad_ckpt: bool = False  # gradient checkpointing
    grad_accu: int = 4  # gradient accumulation
    device: str = 'cpu' # will be set automatically
    dtype: torch.dtype = torch.float32 # will be set automatically

    # data
    train_data: str = 'configs/pt_tok.yaml'
    val_data: str = None
    dataset_type: str = 'webdataset'
    imagenet_train: str = None
    imagenet_val: str = '/mnt/shenzhen2cephfs/mm-base-vision/markywguo/dataset/ImageNet-1k/val/'
    ...
    decoder_rec_depth: int = 27                    # B: 12; L: 24; so: 27
    decoder_rec_num_heads: int = 16                # B: 12; L: 16; so: 16
    decoder_rec_hidden_size: int = 1152            # B: 768; L: 1024; so: 1152
    decoder_rec_intermediate_size: int = 4304      # B: 3072; L: 4096; so: 4304
    ...
    # optimization
    epoch: int = 5  # number of epochs
    local_bs: int = 8  # batch size per device; if this is specified, --global_bs will be ignored
    vae_local_bs: int = 8 # sub-batch size for vae loss calculation
    global_bs: int = 0  # global batch size (exclusive to --local_bs)
    lr: float = 2e-4  # learning rate
    wd: float = 0.02  # weight decay
    disc_lr: float = 1e-5  # disc lr
    disc_wd: float = 0.2
    grad_clip: float = 10  # <=0 for not using grad clip
    ema: float = 0.9999  # ema ratio
    warmup_iter: int = None
    warmup_ep: float = 0.02  # lr warmup: epochs
    disc_start_ep: float = 0.2  # start using disc loss for VAE after xxx epochs;
    disc_warmup_ep: float = 0.02  # disc loss warm up epochs;
    ...
    ld: float = 0.5  # discriminator loss weight; if <0: NO ADAPTIVE WEIGHT
    llecam: float = 0.05  # LeCAM loss weight
    le: float = 0.1  # VQ entropy loss weight
    lq: float = 1.0  # VQ loss weight
    lc: float = 0.0  # CLIP loss weight
    ldistill: float = 1.0  # feature distillation loss weight
    e_temp: float = 0.01
    gada: int = 1
