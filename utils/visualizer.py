...
from data.dataset import pil_load
from utils.dist import for_visualize
from utils.logger import tensorboard_log
...
class Visualizer(object):
    ...
    @for_visualize
    def vis(self, step, png_path='', tb_logger=None) -> Tuple[float, float]:
        if not self.enable:
            return -1., -1.
        vis_stt = time.time()
        ...
        if tb_logger is not None:
            tensorboard_log(tb_logger, {'Vis/Lnll': diff}, step)
        if png_path:
